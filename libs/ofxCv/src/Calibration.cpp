...
namespace ofxCv {
    ...
    bool Calibration::findBoard(cv::Mat img, std::vector<cv::Point2f>& pointBuf, bool refine) {
        ...
        if(patternType == CHESSBOARD) {
            ...
            int chessFlags = cv::CALIB_CB_ADAPTIVE_THRESH;// | CV_CALIB_CB_NORMALIZE_IMAGE;
            ...
            if(found) {
                ...
                if(refine) {
                    ...
                    cornerSubPix(grayMat, pointBuf, subpixelSize,  cv::Size(-1,-1), cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 30, 0.1 ));
                }
            }
        }
    ...
    void Calibration::updateReprojectionError() {
        ...
        for(std::size_t i = 0; i < objectPoints.size(); i++) {
            ...
            double err = norm(cv::Mat(imagePoints[i]), cv::Mat(imagePoints2), cv::NormTypes::NORM_L2);
